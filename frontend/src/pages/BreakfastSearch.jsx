...
import { addBreakfast } from "../redux/app/action";
...
const BreakfastSearch = () => {
  ...
  const handleSearch = () => {
    axios
      .get(`${BACKEND_URL}/foods?q=${query}`)
      .then((res) => setFoods(res.data))
      .catch((err) => console.log(err));
  };

  const addBreakfastHandler = (breakfast) => {
    dispatch(addBreakfast(breakfast));
    navigate("/foods");
  };
                  ...
                  <Box border={"1px solid"} w="60%">
                    {foods.map((el) => (
                      <Box
                        ...
                        m={"auto"}
                        p="1"
                        onClick={() => addBreakfastHandler(el)}
                        cursor="pointer"
                      >
                        <Box
                          fontSize={"small"}
